function updateCountdown() {
  ...
  const hours = Math.floor(
    (timeDiference % (1000 * 60 * 60 * 24)) / (1000 * 60 * 60)
  );
  ...
  document.getElementById(
    "countdown"
  ).textContent = ` ${days} Days, ${hours} Hours, ${minutes} Minutes, ${secconds} Secconds`;
}
...
document.addEventListener("mousemove", (event) => {
  document.body.style.backgroundColor = `rgb(${event.clientX},${event.clientY},${Math.floor((event.clientX)/(event.clientY))})`;
});

let ele = document.getElementById("text");
let texts =
  "As the old year makes way for the new, remember that you are the author of your own story. But the happiest thing is that कि the closing of one chapter is the beggining of another.";
let index = 0;
function LessonS() {
